...
from src.utils.model_utils import get_clones
...
class PolygenDecoderLayer(nn.TransformerDecoderLayer):
    ...
    def forward(
        ...
        if cache is not None:
            saved_key = cache["k"].to(tgt.device)
            saved_value = cache["v"].to(tgt.device)
...
class PolygenDecoder(LightningModule):
    """
...
class TransformerDecoder(LightningModule):
    ...
    def initialize_cache(self, batch_size) -> Dict[str, torch.Tensor]:
        ...
        cache = [
            {"k": k.to(self.device), "v": v.to(self.device)}
