...
def point_sample(points: np.ndarray, npoints: int = 2048):
    if len(points) > npoints:
        idx = np.random.randint(0, len(points), npoints)
    else:
        idx = np.random.randint(0, len(points), npoints - len(points))
...
def farthest_point_sample_numpy(point: np.ndarray, npoint: int = 2048):
    """
    Input:
...
def farthest_point_sample_torch(point: torch.Tensor, npoint: int = 2048):
    """
    Input:
        xyz: pointcloud data, [N, D]
        npoint: number of samples
...
def normalize_vertices_scale(
    vertices: torch.Tensor, return_scale=False
) -> torch.Tensor:
...
def dequantize_verts(verts: torch.Tensor, n_bits: int = 8) -> torch.Tensor:
...
def quantize_process_mesh(
    ...
    faces = [torch.Tensor(f).to(torch.int32) for f in faces]
...
def draw_point_cloud(vis_points, save_path):
